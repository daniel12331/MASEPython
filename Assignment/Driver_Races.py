...
def get_driver_constructor_points(connection,driver_name):
    ...
    driver_id = get_driver_id(driver_name, connection)
    #
    query_driver_average_pitstop = "Select constructor_standings.points as 'constructor_points', driver_standings.points as 'driver_points' from driver_standings Inner join constructor_standings on driver_standings.raceId = constructor_standings.raceId where driverId = {0}".format(driver_id)
    df_mysql = pd.read_sql(query_driver_average_pitstop, con=connection)
...
def get_driver_constructor_wins(connection,driver_name):
    ...
    driver_id = get_driver_id(driver_name, connection)
    #
    query_driver_average_pitstop = "select constructor_standings.wins as 'constructor_wins', driver_standings.wins as 'driver_wins' from driver_standings Inner join constructor_standings on driver_standings.raceId = constructor_standings.raceId where driverId = {0}".format(driver_id)
    df_mysql = pd.read_sql(query_driver_average_pitstop, con=connection)
